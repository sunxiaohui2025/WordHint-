...
check(null, `  Difficulty level buttons in HTML`, popupHtml.includes('cet4') && popupHtml.includes('cet6') && popupHtml.includes('advanced'));
...
check(null, `  chrome.storage.local only (no sync)`, contentJs.includes('storage.local') && !contentJs.includes('storage.sync'));
...
console.log(`  ☐ 点击「收藏」→ 打开插件面板 → 确认单词在学习名单中`);
